...
void check_single_string(Proxy proxy, size_t string_length) {
    ...
    SECTION("overlength null-padded") {
        proxy.create("overlength_nullterm", dataspace, overlength_nullpad).write(value);
        auto expected = std::string(n_chars_overlength, '\0');
        expected.replace(0, value.size(), value.data());
        REQUIRE(proxy.get("overlength_nullterm").template read<std::string>() == expected);
    }

    SECTION("overlength space-padded") {
        proxy.create("overlength_nullterm", dataspace, overlength_spacepad).write(value);
