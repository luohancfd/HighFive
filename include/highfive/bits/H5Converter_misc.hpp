...
/// different input depending on if the strings are fixed or variable length.
...
/// expects a list of pointer to the beginning of each string. Variable length
/// string must be null-terminated; otherwise their length is unknown.
///
/// This buffer hides the difference between fixed and variable length strings
/// by having internal datastructures available for both cases at compile time.
/// The choice which internal buffer to use is made at runtime.
///
/// In a dataset with N fixed-length strings which each are M characters long,
/// the in-memory strings are copied into an internal buffer of size N*M. If
/// null- or space-padded the buffer should be filled with the appropriate
/// character. This is important if the in-memory strings are less than M
/// characters long.
///
/// A dataset with N variable-length strings (all null-terminated) uses uses
/// the internal list of pointers to the beginning of each string. Those
/// pointers can either point to the in-memory string themselves, if those
/// strings are known to be null-terminated. Otherwise the in-memory string are
/// copied to an internal buffer of null-terminated strings; and the pointer
/// point to the start of the string in the internal buffer.
///
/// This class is responsible for arranging the string properly before passing
